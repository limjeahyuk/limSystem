...
| `BottomSheet` | 화면 아래에서 올라오는 시트. `Dialog`와 같은 조합형(`Trigger / Content / Title / Body / Footer / Close / CloseButton`). 핸들을 끌어 닫거나 `Content snapPoints={["40%", "90%"]}` 사이를 이동. `Content width`(기본 `"100%"`). `dismissible={false}`면 바깥 클릭·ESC·드래그로 안 닫힘 |
...
| `TreeView` | 계층 트리. `data: TreeNode[]`(`id/label/icon/badge(숫자·문자면 RED Badge, 노드면 그대로)/disabled/children`), `expanded/defaultExpanded/onExpandedChange`, `selected/onSelect(id)`(싱글), `checkable/checked/onCheckedChange(ids)`(leaf id 기준, 부모는 indeterminate 자동), `color` |
